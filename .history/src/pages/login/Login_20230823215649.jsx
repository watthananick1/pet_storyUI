...
export default function SignInSide() {
    ...
    try {
      ...
      await firebase
        ...
        .then(async (result) => {
          ...
          await loginGoogleCall({ uid: user.uid }, dispatch, history);
        })
        .catch((error) => {
          dispatch({
            type: "LOGIN_FAILURE",
  ...
  const handleSubmit = async (event) => {
    ...
    if (!isValidEmail(email)) {
      Swal.fire("อีเมลไม่ถูกต้อง", "กรุณากรอกอีเมลให้ถูกต้อง", "warning");
      return;
    }
    ...
    try {
      setLoading(true);
      await loginCall({ email: email, password: password }, dispatch, history);
      setMessage({
        severity: "error",
