  ...
  useEffect(() => {
    ...
    const fetchAllTypePets = async () => {
      try {
        ...
        const filteredTypePets = typePetsData.map((typePets) => {
          const filterTypePet = typePets.tags.filter((tag) => tag.status === true);
          return { ...typePets, tags: filterTypePet };
        });
    
        console.log("Filtered TypePets:", filteredTypePets);
    
        // Perform other operations with filteredTypePets as needed
    
        return filteredTypePets;
      } catch (error) {
