...
export default function Sidebar() {
  ...
  const handleTypePet = (type) => {
    history.push("/sort/type/" + type?.nameType);
  };

  const handleDeleteTypePet = async (typeName) => {
    try {
      Swal.fire({
        title: `คุณแน่ใจหรือไม่ที่จะลบ ${typeName} ?`,
        ...
        if (result.isConfirmed) {
          await UsersCollection.doc(user.member_id).update({
            typePets: firebase.firestore.FieldValue.arrayRemove(typeName),
          });
          setTypePets((prevTypePets) =>
            prevTypePets.filter((type) => type.nameType !== typeName)
          );
          Swal.fire("ลบสำเร็จ!", `ประเภทสัตว์${typeName}ของคุณถูกลบแล้ว`, "success");
        }
      });
    } catch (error) {
  ...
  const handleSubmit = async () => {
    try {
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          const userDoc = await UsersCollection.doc(user.member_id).get();
          const updatedTypePets = userDoc.data().typePets;
          
          const typeDocsPromises = updatedTypePets.map(async (tag) => {
            const querySnapshot = await typePetsCollection.where("typePets", "==", tag).get();
            return querySnapshot.docs.map(doc => doc.data());
          });
  
          const typeDocs = await Promise.all(typeDocsPromises);
  
          const newTypePets = typeDocs.flat(); // Combine arrays of typeDocs into a single array
          setTypePets(newTypePets);
        }
      });
    } catch (error) {
          ...
          <List sx={{ width: "100%" }}>
            {typePets.map((type, index) => (
              <React.Fragment key={index}>
                <ListItem
                  button
                  onClick={() => handleTypePet(type)}
                  secondaryAction={
                    <IconButton
                      edge="end"
                      aria-label="delete"
                      onClick={() => handleDeleteTypePet(type.nameType)}
                    >
                      <DeleteIcon />
                    </IconButton>
                  }
