...
export default function Sidebar() {
  ...
  useEffect(() => {
    const unsubscribe = UsersCollection.onSnapshot(async (snapshot) => {
      const updatedCloseFriends = await Promise.all(
        snapshot.docs.map(async (doc) => {
          const userData = doc.data();
          const userDoc = await typePetsCollection.doc(userData.member_id).get();

          if (userDoc.exists) {
            const tagpet = userDoc.data().typePets;

            return {
              id: postData.member_id, // ตัวอย่างข้อมูลที่คุณต้องการส่งให้ CloseFriend component
              nameType: userData.nameType,
              imgPet: userData.imgPet,
              // อาจเพิ่มข้อมูลอื่น ๆ ตามที่คุณต้องการ
            };
          } else {
            return null;
          }
      ...
      const validTypePets = updatedCloseFriends.filter(
        (friend) => friend !== null
      );
      setTypePets(validTypePets);
    });
