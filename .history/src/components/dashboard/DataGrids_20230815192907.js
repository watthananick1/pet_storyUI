...
const ReportUsersCollection = firestore.collection("Users");
...
const fetchReportUser = usersCollection.onSnapshot(async (snapshot) => {
...
const fetchUser = usersCollection.onSnapshot(async (snapshot) => {
  const updatedTypePets = await Promise.all(
    snapshot.docs.map(async (doc) => {
      const userData = doc.data();
