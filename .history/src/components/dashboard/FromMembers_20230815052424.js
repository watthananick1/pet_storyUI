...
function EditData(id, profilePicture, firstName, lastName, email, statusUser, status) {
...
export default function FromMembers({ selectedMember }) {
  ...
  const [firstName, setNameType] = useState("");
  ...
  useEffect(() => {
    if (selectedMember) {
      setNameType(selectedMember.firstName);
      setStatus(selectedMember.status === 'active');
      setImagePreview(selectedMember.imgPet);
    }
  }, [selectedTypePet]);
