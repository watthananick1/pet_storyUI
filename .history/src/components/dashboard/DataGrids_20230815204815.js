  ...
  },
  {
    ...
    renderCell: (params) => (
      <Typography variant="subtitle1">
        {format(params.value.toDate(), "MM/dd/yyyy HH:mm:ss")}
      </Typography>
    ),
  },
  {
    ...
    renderCell: (params) => (
      <Typography variant="subtitle1">
        {format(params.value.toDate(), "MM/dd/yyyy")}
      </Typography>
    ),
  },
  {
        ...
        async (snapshot) => {
          const updatedTypePets = await Promise.all(
            snapshot.docs.map(async (doc) => {
              ...
              const dataReported = await getUserData(reportUserData.reported_member_id);
              const dataReporter = await getUserData(reportUserData.reporter_id);
