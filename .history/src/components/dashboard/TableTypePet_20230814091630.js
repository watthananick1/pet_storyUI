...
const columns = [
    { field: 'imgPet', headerName: 'IMAGE', width: 90, renderCell: (params) => (
        <img
          src={params.value} // ตัวแปร value จะมีค่าเป็น URL ของรูปภาพ
          alt=""
          style={{ width: '100%', height: 'auto' }} // ปรับขนาดรูปภาพตามต้องการ
        />
      ), },
    {
      field: 'nameType',
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        dataRows.map((tag) => {
          data.push(geteData(tag.id_TypePet, tag.imgPet, tag.nameType, tag.status));
        });
