...
function geteUserData( id ) async {
  const fetchUser = await usersCollection.onSnapshot(async (snapshot) => {
...
export default function DataGridSection() {
  ...
  useEffect(() => {
    try {
      const fetchReportUser = reportUsersCollection.onSnapshot(async (snapshot) => {
        const updatedTypePets = await Promise.all(
          snapshot.docs.map(async (doc) => {
