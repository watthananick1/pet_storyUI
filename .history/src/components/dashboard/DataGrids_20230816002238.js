    ...
    try {
      const fetchReportUser = reportUsersCollection.onSnapshot(
        async (snapshot) => {
          ...
          switch (sort) {
            case "all":
              return updatedReportUsers.filter((user) => {
                if (user && user.startus) {
                  const userStatus = user.status === "in progress"
                  return userStatus
                }
                return false;
              });
            case "in progress":
              return setRows(updatedReportUsers);
            case "is checking":
              return updatedReportUsers.filter((user) => {
                if (user && user.startus) {
                  const userStatus = user.status === "is checking"
                  return userStatus
                }
                return false;
              });
            case "completed":
              return updatedReportUsers.filter((user) => {
                if (user && user.startus) {
                  const userStatus = user.status === "completed"
                  return userStatus
                }
                return false;
              });
            case "cancel":
              return updatedReportUsers.filter((user) => {
                if (user && user.startus) {
                  const userStatus = user.status === "cancel"
                  return userStatus
                }
                return false;
              });
          ...
          setRows(updatedReportUsers);
          setNewReportUser(updatedReportUsers);
        }
      );
