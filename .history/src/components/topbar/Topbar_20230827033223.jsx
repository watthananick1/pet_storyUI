...
export default function PrimarySearchAppBar() {
  ...
  const [menuanchorElC, setMenuAnchorElC] = React.useState(null);
  const [loading, setLoading] = React.useState(false);
  const open = Boolean(anchorEl);
  const menuopen = Boolean(menuanchorEl);
  const menuCopen = Boolean(menuanchorElC);
  ...
  const handleAddTypePet = () => {
    // Logic to add TypePet
    handleCloseAddDialog(); // Close the dialog after adding
  };
  ...
  const handleClickCOpen = (scrollType) => () => {
    setHomeCopen(true);
    setScroll(scrollType);
  };
  ...
  const handleHomeClick = (event) => {
    setMenuAnchorElC(event.currentTarget);
  };
  const handleHomeClose = () => {
    setMenuAnchorElC(null);
  };
  ...
  const handleSearchPostClick = (id) => {
    handleClose();
  ...
  const mobileMenuHomeId = "primary-search-account-menu-mobileHome";
  const renderMobileHomeMenu = (
    <Menu
      anchorEl={menuanchorElC}
      id={mobileMenuHomeId}
      open={menuCopen}
      ...
      <MenuItem>
        <Tooltip title="Add TypePet" placement="right-end">
          <Fab
            ...
            onClick={handleClickCOpen("paper")}
          >
            <AddIcon />
          </Fab>
        </Tooltip>
                ...
                renderOption={(props, option) => {
                  ...
                  if (option.type === "Post") {
                    return (
                      <li
                        {...props}
                        onClick={() => handleSearchPostClick(option.id)}
                      >
                        <Stack direction="row" alignItems="center">
                          <span>{`Post `<PostText>${option?.content}<PostText></PostText>}</span>
                        </Stack>
                      </li>
                    );
                  }
