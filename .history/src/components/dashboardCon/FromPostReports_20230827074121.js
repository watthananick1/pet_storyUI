...
import FromPosts from "./FromPostsData";
...
const reportPostCollection = firestore.collection("Report_Post");
...
export default function FromPostReports({ selectedMember }) {
  ...
  useEffect(() => {
    if (selectedMember) {
      ...
      setFirstName(selectedMember.report_firstName);
      setLastName(selectedMember.report_lastName);
      ...
      setPostComment(selectedMember.post_content);
    }
  }, [selectedMember]);
  ...
  const handleUpdateReportPost = async () => {
    try {
      setLoading(true);
      await reportPostCollection.doc(selectedMember.id).update({
        ...
        >
          <FormGroup>
            ...
            <Grid item xs={12} md={12}>
              ...
              <Typography sx={{ ml: 2 }} variant="caption" gutterBottom>
                เมื่อ: {dateTime}
              </Typography>
            </Grid>
