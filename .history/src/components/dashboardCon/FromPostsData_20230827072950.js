...
const Report_PostCollection = firestore.collection("Report_Post");
...
export default function FromPosts({ selectedMember }) {
  ...
  const [onImage, setOnImage] = useState("image");
  ...
  const handleUpdateReportUser = async () => {
    try {
      setLoading(true);
      await postsCollection.doc(selectedMember.post_id).update({
        status: "private",
        statusPost: status,
  ...
  const handleUnBlogReportUser = async () => {
    try {
      setLoading(true);
      await postsCollection.doc(selectedMember.post_id).update({
        status: "followers",
  ...
  useEffect(async () => {
    if (selectedMember) {
      ...
      setStatus(postData.statusPost);
      setOnImage(postData.title);
      ...
      {selectedMember ? (
        <Paper elevation={0} sx={{ p: 2 }}>
          <Dialog open={openBlockDialog} onClose={handleCloseBlockDialog}>
            <DialogTitle>บล็อกโพสต์</DialogTitle>
            <DialogContent>
              <DialogContentText>
                    ...
                    <IOSSwitch
                      ...
                      onChange={(e) => {
                        ...
                        if (!e.target.checked) {
                          ...
                          handleUnBlogReportUser();
                        }
                      }}
                    />
                  }
                  label={status ? "normal" : "blog"}
                />
              </Grid>
              <Grid item xs={12} md={12}>
                <LoadingButton
