...
import { FilterPost } from "../../context/AuthActions";
...
function BasicSelect({ user }) {
  ...
  const [typePets, setTypePets] = useState(filterText);

  const handleChange = (event) => {
    setTypePets(event.target.value);
    dispatch(FilterPost(event.target.value, filterPost));
  };

  return (
    <Box sx={{ width: "100%" }}>
      <FormControl fullWidth>
        <InputLabel id="demo-simple-select-label">Type Pets</InputLabel>
        <Select
...
function CustomizedInputBase() {
  const [type, setTypePet] = useState("");
  const {
    user: currentUser,
    filter: filterPost,
    filterText,
  ...
  const handleClick = async () => {
    dispatch(FilterPost(searchInput, filterPost));
  };
  return (
    <Paper
      component="form"
    ...
    >
      <InputBase
        sx={{ ml: 1, flex: 1 }}
        placeholder="Search Type Pet"
        inputProps={{ "aria-label": "Search Type Pet" }}
        value={searchInput}
...
export function Rightbar({ user }) {
  ...
  const handlePrivacyChange = (event, value) => {
    if (value) {
      ...
      dispatch(FilterPost("", value.value));
    } else {
      setPrivacy("normal");
      dispatch(FilterPost("", "normal"));
    }
  };
