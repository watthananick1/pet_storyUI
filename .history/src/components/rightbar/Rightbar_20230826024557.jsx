...
export function RightbarR({ user }) {
  ...
  useEffect(() => {
    const getFriends = async () => {
      try {
        ...
        const friendPromises = currentUserDoc.data().followings.map(async (friend) => {
          const friendRef = firestore.collection("Users").doc(friend?.member_id);
  ...
  const ProfileRightbar = () => {
    return (
      <>
        <Box>
          <Card className="Wrapper" sx={{ ml: 2, mr: 2 }}>
            <>
              <AvatarGroup max={4}>
                {friends.map((friend) => (
                  <Avatar
