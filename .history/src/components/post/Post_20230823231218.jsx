...
export default function Post({ isPost, onPostUpdate, indexPost }) {
    ...
    const fetchComments = async () => {
      try {
        const resComments = await axios.get(
          `${path}/api/comments/${post.id}/Comments`, // ใช้ post.id เป็น postId เพื่อดึงความคิดเห็นของโพสต์ที่เกี่ยวข้อง
          {
            headers: {
              Authorization: `Bearer ${token}`,
            },
  ...
  }, [post.id]); // ใช้ post.id เป็น dependency เพื่อให้ useEffect เรียกใช้ใหม่เมื่อ postId เปลี่ยน
