...
export default function Post({ isPost, onPostUpdate, indexPost }) {
  ...
  const handlePostUpdate = (updatedPost, type) => {
    ...
    if (type === "Post") {
      ...
      setComments((prevComments) => {
        const updatedComments = [...prevComments]; // Create a shallow copy of the existing comments
        updatedComments[0] = [...updatedComments[0], updatedPost]; // Add the new comment to the first nested array
        return updatedComments;
      });
      setLoadingComment(false);
    } else {
