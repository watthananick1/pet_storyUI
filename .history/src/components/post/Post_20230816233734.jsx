...
export default function Post({ isPost, onPostUpdate, indexPost }) {
  ...
  useEffect(() => {
    setStatusAdmin(post.post_Status.includes("ADMIN") || post.post_Status.includes("ADMINCON"));
  }, [post.post_Status]);
  ...
  const likeHandler = async () => {
    try {
      ...
      if (response.status === 200) {
        ...
        dispatch(
          Messageupdate("Failed to like post.", true, "error")
        );
      }
    } catch (err) {
      dispatch(
  ...
  const handleDeleteComment = async () => {
    ...
    try {
      ...
      handleClose();
      dispatch(
        Messageupdate("Delete comments successfully.", true, "success")
      );
    }
  };
                      ...
                      <div className="postCommentProfile">
                        ...
                        >
                          {user.member_id === commentIdUser ? (
                            ...
                            <MenuItem key="report" onClick={handleReportUser(comment.id)}>
                              <span>
                                <ReportIcon fontSize="small" />
                              </span>
