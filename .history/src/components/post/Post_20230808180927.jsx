...
export default function Post({ isPost, onPostUpdate, indexPost }) {
  ...
  const likeHandler = async () => {
    try {
      ...
      if (response.status === 200) {
        ...
        dispatch({ type: "MESSAGE_UPDATE", message: "Failed to like post.", open: true, severity: "error"});
      }
    } catch (err) {
      dispatch({ type: "MESSAGE_UPDATE", message: "Failed to like post.", open: true, severity: "error"});
      console.log(err);
  ...
  useEffect(() => {
    ...
    const fetchComments = async () => {
      try {
        ...
        dispatch({ type: "MESSAGE_UPDATE", message: "Success comments post.", open: true, severity: "success"});
        dispatch(
          Messageupdate("Success comments post.", true, "success")
        );
      } catch (err) {
        dispatch(
          Messageupdate("Failed comments post.", true, "error")
        );
        console.log(err);
      } finally {
    ...
    const unsubscribePost = postRef.onSnapshot((postSnapshot) => {
      ...
      unsubscribeUser = userRef.onSnapshot((userSnapshot) => {
        if (!userSnapshot.empty) {
          ...
          dispatch({ type: "MESSAGE_UPDATE", message: "Failed no matching user post.", open: true, severity: "error"});
  ...
  const handleDeletePost = async () => {
    ...
    try {
      ...
      dispatch(
        Messageupdate("Failed Delete Post.", true, "error")
      );
      console.log(err);
    } finally {
      dispatch(
