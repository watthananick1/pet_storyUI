...
export default function Share({ onNewPost }) {
  ...
  const submitHandler = async (e) => {
    ...
    //   dispatch(
    //     Messageupdate(
    //       "Cannot share post. Socket is not connected.",
    //       true,
    //       "error"
    //     )
    //   );
    ...
    try {
      socket = io.connect(process.env.PATH_ID);
      ...
      dispatch(Messageupdate("Failed Share Post.", true, "error"));
      console.log(err);
    } finally {
  ...
  useEffect(() => {
    const getTypePets = async () => {
      try {
        ...
        dispatch(Messageupdate("Failed get type pets.", true, "error"));
        console.log(err);
      }
    };
    getTypePets();
