...
export default function Share({ onNewPost }) {
  ...
  useEffect(() => {
    ...
    socket.on("connect", () => {
      setSocketConnected(true); // ตั้งค่าสถานะการเชื่อมต่อเป็น true เมื่อ socket ได้เชื่อมต่อ
    });
    socket.on("disconnect", () => {
      setSocketConnected(false); // ตั้งค่าสถานะการเชื่อมต่อเป็น false เมื่อ socket ถูกตัดการเชื่อมต่อ
    });

    return () => {
      socket.disconnect(); // ตัดการเชื่อมต่อ socket เมื่อคอมโพเนนต์ถูกทำลาย
    };
  }, []);
  ...
  const submitHandler = async (e) => {
    ...
    if (!socketConnected) {
      dispatch(Messageupdate("Cannot share post. Socket is not connected.", true, "error"));
      return; // ถ้า socket ไม่ได้เชื่อมต่อ จะไม่ทำการส่งโพสต์
    }
    ...
    try {
      ...
      dispatch(
        Messageupdate("Success Share Post.", true, "success")
      );
      setLoading(false);
    }
  };
  ...
  useEffect(() => {
    const getTypePets = async () => {
      try {
        ...
        dispatch(
          Messageupdate("Failed get type pets.", true, "error")
        );
        console.log(err);
      }
    };
