    ...
    if (onContent !== content) {
      try {
        ...
        if (onTitle === "Post") {
          ...
          dispatch(
            Messageupdate("Success comment post.", true, "success")
          );
        } else {
        ...
        setLoading(false);
        dispatch(
          Messageupdate("Success Add Content Post.", true, "success")
        );
      }
    } else {
      dispatch(
        Messageupdate("Failed Edit Content.", true, "error")
      );
  ...
  const handleSaveChanges = async () => {
    if (onContent !== content) {
      try {
        ...
        dispatch({ type: "MESSAGE_UPDATE", message: "Failed report Post.", open: true, severity: "error"});
        dispatch(
          Messageupdate("Failed report Post.", true, "error")
        );
        console.log(err);
      } finally {
        setLoading(false);
        dispatch({ type: "MESSAGE_UPDATE", message: "Success report Post.", open: true, severity: "success"});
      }
    } else {
      console.log("Err Edit Content");
