    ...
    if (onContent !== content) {
      try {
        ...
        if (onTitle === "Post") {
          ...
          endpoint = `${path}/api/report/reportUser`;
          rePortData = {
            reporter_id: onContentID,
            reported_member_id: userId,
            comment: content,
            status: "in progress",
          };
        }
        ...
        dispatch(Messageupdate("Failed report Post.", true, "error"));
        console.log(err);
      } finally {
        setLoading(false);
        dispatch(Messageupdate(`Success report Post.`, true, "success"));
      }
    } else {
      console.log("Err Edit Content");
      dispatch(Messageupdate("Failed Edit Content.", true, "error"));
