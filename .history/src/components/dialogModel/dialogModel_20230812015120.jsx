...
export default function FilePreviewer({ onClose }) {
  ...
  const getCroppedProfileImageBlob = async (sourceImageUrl, crop, profileSize) => {
    ...
    return new Promise((resolve) => {
      image.onload = () => {
        ...
        const offsetX = (scaledWidth - profileSize) / 2;
        const offsetY = (scaledHeight - profileSize) / 2;
  ...
  const handleAvatarUpload = async () => {
    ...
    try {
      ...
      const croppedImageBlob = await getCroppedProfileImageBlob(imagePreview, crop, profileSize);
      ...
      dispatch(Messageupdate("Error uploading profile picture.", true, "error"));
    } finally {
      dispatch(Messageupdate("Profile picture updated successfully.", true, "success"));
      setLoading(false);
    }
  };
