  ...
  const handleSubmit = async (event) => {
    ...
    if (hasChanges) {
      try {
        ...
        userDoc = await usersRef.doc(currentUser.member_id).update({
        ...
        Swal.fire({
          icon: "error",
          title: "Oops...",
          text: `เกิดปัญหาขึ้นระหว่างบันทึก กรุณาลองใหม่! ${userDoc} `,
        });
      }
    } else {
      Swal.fire(
                ...
                <Grid item xs={12} md={12} lg={12}>
                  <LocalizationProvider dateAdapter={AdapterDayjs}>
                    <DatePicker
                      ...
                      defaultValue={formatDate(currentUser.dateOfBirth) ?? formatDate(currentUser.dateOfBirth) dataUser.lastName}
                    />
