  ...
  const handleSubmit = async (event) => {
    ...
    if (hasChanges) {
      try {
        ...
        console.log(dataObj);
        localStorage.setItem("user", JSON.stringify(dataUser, ...currentUser.token));
        dispatch({ type: "UPDATE_USER", payload: currentUser.token, ...currentUser.token });
        ...
        Swal.fire({
          icon: "error",
          title: "Oops...",
          text: `เกิดปัญหาขึ้นระหว่างบันทึก กรุณาลองใหม่! ${currentUser.token} `,
        });
                ...
                <Grid item xs={12} md={12} lg={12}>
                  <LocalizationProvider dateAdapter={AdapterDayjs}>
                    <DatePicker
                      ...
                      defaultValue={formatDate(currentUser.dateOfBirth) ?? formatDate(dataUser.dateOfBirth)}
                    />
