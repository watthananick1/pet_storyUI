...
export default function Feed({ firstName, onProfile }) {
  ...
  const handleRefresh = (key) => {
    ...
    setShowNewPosts((prevNewPosts) => [...newPosts, ...prevNewPosts]);
    setNewPosts([]); // ล้างค่า newPosts
  };
  ...
  useEffect(() => {
    if (newPosts.length > 0) {
      ...
      if (hasDifferentMemberId) {
        ...
        //setPosts((prevPosts) => [...newPosts, ...prevPosts]); // Add new posts to the beginning
      ...
      <SnackbarProvider maxSnack={3}>
        <div className="feedWrapper">
          ...
          {loading ? (
            ...
            <>
              {showNewPosts.length > 0 &&
                showNewPosts.map((p, i) => (
