...
export default function Feed({ firstName, onProfile }) {
  ...
  const [posts, setPosts] = useState([]); // เพิ่มบรรทัดนี้
  const [loading, setLoading] = useState(true); // เพิ่มบรรทัดนี้
  ...
  useEffect(() => {
    const unsubscribe = postsCollection
      ...
      .onSnapshot((snapshot) => {
        ...
        dispatch({ type: "FETCH_POSTS_SUCCESS", payload: newPosts }); // ใช้ dispatch เพื่อเก็บข้อมูลโพสต์ลงใน Redux store
      });
