      ...
      .onSnapshot(async (snapshot) => {
        ...
        const newPosts = await Promise.all(snapshot.docs.map(async (doc) => {
          ...
          if (userDoc.exists) {
            ...
            return null; // ไม่ต้องเพิ่มโพสต์ที่ไม่มีผู้ใช้
          }
        }));
        
        const filteredPosts = newPosts.filter((post) => {
          return post !== null; // กรองออกเพื่อลบโพสต์ที่ไม่มีผู้ใช้
        }).filter((post) => {
        ...
        setNewPosts(filteredPosts); // อัพเดท state newPosts เพื่อเก็บโพสต์ที่เพิ่มใหม่
      });
