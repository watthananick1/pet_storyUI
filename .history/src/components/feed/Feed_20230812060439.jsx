...
export default function Feed({ firstName, onProfile }) {
  ...
  useEffect(() => {
    ...
    const handleNewPost = (newPost) => {
      setPosts((prevPosts) => [newPost, ...prevPosts]); // เพิ่มโพสต์ใหม่ไปยังอาร์เรย์และเรียงลำดับใหม่
    };
    ...
    const unsubscribe = postsCollection
      ...
      .onSnapshot((snapshot) => {
        ...
        const posts = snapshot.docs
          .map((doc) => ({
            ...doc.data(), // เนื่องจากคุณต้องการคัดลอกข้อมูลทั้งหมดจาก document
            const user = UsersCollection.docs(doc.data().member_id).get
            firstName: user.firstName,
            lastName: user.lastName,
            post_Status: user.statusUser,
            profilePicture: user.profilePicture,
          }))
          .filter((post) => {
            const postTime = new Date(post.createdAt.seconds * 1000).getTime();
            return postTime > currentTime - 60000; // เลือกโพสต์ที่ถูกสร้างใหม่ในช่วง 1 นาทีที่ผ่านมา
          });
  
        setNewPosts(posts); // อัพเดท state newPosts เพื่อเก็บโพสต์ที่เพิ่มใหม่
      });
