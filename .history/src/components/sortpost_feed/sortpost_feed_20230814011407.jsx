...
export default function Sortpost_feed({ firstName, sort }) {
  ...
  const fetchNewsPosts = async () => {
    ...
    try {
      ...
      const filteredPosts = res.data.filter((post) => post.tagpet.includes("news"));
      setPosts(filteredPosts);
    } catch (err) {
      if (axios.isCancel(err)) {
  ...
  }, [firstName, user.member_id, newPosts, sort]);
    ...
    try {
      const unsubscribe = postsCollection
        ...
        .onSnapshot(async (snapshot) => {
          ...
          const filteredVideoPosts = filteredPosts.filter((post) => post.title === "video");
            //console.log("filteredPosts", filteredPosts);
